...
const rolePermissions = {
  admin: ['products', 'orders', 'inventory', 'users', 'analytics', 'settings', 'notifications'],
...
// Sidebar Component
const Sidebar = ({ isOpen, setIsOpen, activeSection, setActiveSection, userRole }) => {
  ...
  const menuItems = [
    ...
    { id: 'products', label: 'Products', icon: Camera, color: 'text-purple-600' },
    { id: 'orders', label: 'Orders', icon: ShoppingCart, color: 'text-green-600' },
    { id: 'inventory', label: 'Inventory', icon: Package, color: 'text-orange-600' },
    { id: 'users', label: 'Users', icon: Users, color: 'text-red-600' },
        ...
        <nav className="mt-6 px-3">
          {allowedItems.map((item) => (
            <button
              ...
              `}
            >
              <item.icon className={`h-5 w-5 ${activeSection === item.id ? item.color : 'text-gray-500'}`} />
              <span className={`font-medium ${activeSection === item.id ? 'text-gray-900' : 'text-gray-600'}`}>
                {item.label}
              </span>
            </button>
          ))}
