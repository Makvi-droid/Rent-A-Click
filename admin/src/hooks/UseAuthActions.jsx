// hooks/useAuthActions.js - FIXED VERSION
...
function UseAuthActions({
  ...
  // FIXED: Enhanced Google SSO with proper data structure
  const handleGoogleSSO = async () => {
    ...
    try {
      ...
      console.log("Google user object:", user); // Debug log

      // Check if user document exists in ADMIN collection
      const userRef = doc(firestore, "admin", user.uid);
      const docSnap = await getDoc(userRef);

      // FIXED: Complete user data structure for both new and existing users
      const baseUserData = {
        ...
        provider: "google",
        twoFA: false,
        isEmailVerified: user.emailVerified || true, // Google users are pre-verified
        profileComplete: true, // Mark as complete for Google users
        updatedAt: serverTimestamp(),
        lastLoginAt: serverTimestamp(),
        // FIXED: Add missing fields that dashboard might expect
      ...
      if (!docSnap.exists()) {
        ...
        // Existing Google user - update login info and ensure complete profile
      ...
      // FIXED: Navigate after a shorter delay
      setTimeout(() => {
        navigate('/adminDashBoard'); // FIXED: Use kebab-case route (common pattern)
      }, 1000);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isSignUp) {
        ...
        // FIXED: Create complete user document
        const userData = {
          ...
          profileComplete: true, // Mark as complete for email signups
        ...
        console.log("Account created in admin collection:", formData.email);
        
        setTimeout(() => {
          navigate('/adminDashBoard'); // FIXED: Consistent route
        }, 2000);
        ...
        // Update last login in ADMIN collection
        const userRef = doc(firestore, "admin", user.uid);
        ...
        console.log("User logged in from admin collection:", formData.email);
