...
const RealtimeStats = () => {
    ...
    const customersUnsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        setStats((prev) => ({
          ...
          activeUsers: customers.filter((doc) => {
            const customerData = doc.data();
            // Example: customers active in last 30 days
            const lastActive = customerData.lastActive?.toDate();
    ...
    const productsUnsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        const outOfStock = products.filter((doc) => {
          ...
          return (
            productData.stock === 0 || productData.status === "out_of_stock"
          );
        }).length;
    ...
    const checkoutsUnsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        checkouts.forEach((doc) => {
          ...
          const amount =
            checkoutData.pricing?.total || checkoutData.amount || 0;
  ...
  const statCards = [
    ...
    },
    {
      ...
      color: "bg-green-500",
      changeColor:
        stats.outOfStockProducts > 0 ? "text-red-600" : "text-gray-500",
    },
    {
      title: "Total Orders",
