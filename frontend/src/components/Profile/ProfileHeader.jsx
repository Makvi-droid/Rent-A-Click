    ...
    <div className="relative bg-gradient-to-br from-gray-900 via-purple-900/20 to-pink-900/20 rounded-2xl p-8 border border-purple-500/30 overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col md:flex-row items-center md:items-start space-y-6 md:space-y-0 md:space-x-8">
        ...
        >
          ...
          {/* Upload overlay */}
          <div className={`absolute inset-0 rounded-full bg-black/60 flex items-center justify-center transition-all duration-300 ${
        ...
        <div className="flex-1 text-center md:text-left">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
            ...
            <button
              onClick={onEdit}
              className="mt-4 md:mt-0 bg-gradient-to-r from-purple-500 to-pink-500 text-white px-6 py-2 rounded-full font-medium transform hover:scale-105 transition-all duration-300 shadow-lg hover:shadow-purple-500/25 group relative overflow-hidden"
            >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: 'Rentals', value: user.stats.rentals, icon: Camera, color: 'purple' },
              { label: 'Reviews', value: user.stats.reviews, icon: Star, color: 'yellow' },
              { label: 'Rating', value: user.stats.rating, icon: Award, color: 'blue' },
              { label: 'Saved', value: user.stats.saved, icon: Heart, color: 'pink' }
            ].map((stat, index) => (
              <div key={stat.label} className="text-center p-3 rounded-lg bg-gray-800/50 border border-gray-700/50 hover:border-purple-500/30 transition-all duration-300 group">
