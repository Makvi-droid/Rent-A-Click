        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 relative">
          <div className="flex justify-between items-center h-20">
            ...
            <div className="hidden md:flex items-center space-x-8">
              {['Services', 'Features', 'About'].map((item, index) => (
                <a 
                  key={item}
              ...
              <button className="relative bg-gradient-to-r from-purple-500 to-pink-500 text-white px-8 py-3 rounded-full font-medium overflow-hidden group transform hover:scale-105 transition-all duration-300 shadow-lg hover:shadow-purple-500/25" onClick={() => navigate("/auth")}>
                ...
                {/* Border glow */}
                <div className="absolute inset-0 rounded-full border-2 border-transparent bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-border opacity-0 group-hover:opacity-100 transition-opacity duration-300" 
                     style={{ padding: '2px', background: 'linear-gradient(45deg, #a855f7, #ec4899) border-box' }} />
              </button>
            </div>
          ...
          <div className="px-6 py-6 space-y-6 relative z-10">
            {['Services', 'Features', 'About'].map((item, index) => (
              <a 
                key={item}
                href={`#${item.toLowerCase()}`} 
                className="block text-gray-300 hover:text-white transition-all duration-300 py-3 px-4 rounded-lg hover:bg-gradient-to-r hover:from-purple-500/20 hover:to-pink-500/20 border border-transparent hover:border-purple-400/30 transform hover:translate-x-2"
                style={{ 
