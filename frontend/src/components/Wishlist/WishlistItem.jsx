...
const WishlistItem = ({ item, onRemove, onAddToCart, viewMode }) => {
  ...
  // Updated handleAddToCart in WishlistItem component
  const handleAddToCart = async (item) => {
    ...
    try {
      if (onAddToCart && typeof onAddToCart === "function") {
        ...
        // REMOVED: Don't show success toast here anymore since the parent component
        // will handle both success and wishlist removal messages
      } else {
      ...
      showError(
        `Failed to add ${item.name || "item"} to cart: ${error.message}`
      );
    } finally {
  ...
  if (viewMode === "list") {
    return (
      <div
        className={`bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-1 p-6 ${
