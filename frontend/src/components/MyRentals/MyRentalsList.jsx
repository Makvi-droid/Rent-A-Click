// components/MyRentals/MyRentalsList.jsx
...
const MyRentalsList = ({ rentals, formatCurrency, isVisible }) => {
              ...
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-3">
                    {/* Status Badge */}
                    <div className={`flex items-center space-x-2 px-3 py-1 rounded-full text-xs font-medium ${statusInfo.bgColor} ${statusInfo.borderColor} ${statusInfo.color} border`}>
                    ...
                    <div className="text-sm text-gray-400">
                      Order <span className="text-white font-mono">{rental.id}</span>
                    </div>

                    {/* Remaining Days for Active Rentals */}
                    {statusInfo.status === 'active' && remainingDays !== null && remainingDays > 0 && (
                  ...
                  <div className="flex items-center text-sm text-gray-300 mb-3">
                    ...
                    {rental.rentalDetails?.rentalDays && (
                      <span className="ml-2 text-gray-400">
                        ({rental.rentalDetails.rentalDays} day{rental.rentalDetails.rentalDays > 1 ? 's' : ''})
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center text-sm text-gray-400">
                    ...
                    <span>
                      ...
                      {rental.items && rental.items.length > 0 && (
                        <span className="ml-1">
                          • {rental.items[0].name}
                          {rental.items.length > 1 && ` +${rental.items.length - 1} more`}
                        </span>
                      )}
                    </span>
                  </div>
                ...
                <div className="flex flex-col lg:items-end gap-3">
                  <div className="text-right">
                    ...
                    <div className="text-sm text-gray-400">
                      {rental.paymentInfo?.method === 'paypal' ? 'Paid via PayPal' : 'Cash Payment'}
                    </div>
                  </div>
            ...
            {isExpanded && (
              <div className="border-t border-white/10 bg-white/5">
                <div className="p-6 space-y-6">
                    ...
                    <div>
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          <span className="text-gray-400">Method:</span>
                          <span className="text-white capitalize">
                            {rental.paymentInfo?.method === 'paypal' ? 'PayPal' : 'Cash'}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-400">Status:</span>
                          <span className={`${rental.paymentStatus === 'paid' ? 'text-green-400' : 'text-yellow-400'}`}>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {rental.items?.map((item, itemIndex) => (
                        <div key={itemIndex} className="bg-white/5 border border-white/10 rounded-lg p-4">
                          <div className="flex items-center space-x-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="text-sm text-gray-400">
                                Qty: {item.quantity} • {formatCurrency(item.dailyRate)}/day
                              </div>
                  ...
                  {/* Pricing Breakdown */}
                  <div>
                    <h4 className="text-white font-semibold mb-3">Pricing Breakdown</h4>
                    <div className="bg-white/5 border border-white/10 rounded-lg p-4">
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          <span className="text-gray-400">Subtotal:</span>
                          <span className="text-white">{formatCurrency(rental.pricing?.subtotal || 0)}</span>
                        </div>
                        {rental.pricing?.deliveryFee > 0 && (
                          <div className="flex justify-between">
                            <span className="text-gray-400">Delivery Fee:</span>
                            <span className="text-white">{formatCurrency(rental.pricing.deliveryFee)}</span>
                          </div>
                        )}
                        {rental.pricing?.insuranceFee > 0 && (
                          <div className="flex justify-between">
                            <span className="text-gray-400">Insurance:</span>
                            <span className="text-white">{formatCurrency(rental.pricing.insuranceFee)}</span>
                          </div>
                        )}
                        {rental.pricing?.tax > 0 && (
                          <div className="flex justify-between">
                            <span className="text-gray-400">Tax (12%):</span>
                            <span className="text-white">{formatCurrency(rental.pricing.tax)}</span>
                          </div>
                        )}
                        <div className="border-t border-white/10 pt-2 mt-2">
                          <div className="flex justify-between font-semibold">
                            <span className="text-white">Total:</span>
                            <span className="text-white">{formatCurrency(rental.pricing?.total || 0)}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  {/* Additional Options */}
                  {(rental.options?.insurance || rental.options?.specialInstructions) && (
                    <div>
                      <h4 className="text-white font-semibold mb-3">Additional Options</h4>
                      <div className="space-y-2 text-sm">
                        ...
                        {rental.options.specialInstructions && (
                          <div>
                            ...
                            <p className="text-white mt-1">{rental.options.specialInstructions}</p>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="text-xs text-gray-500">
                    <div>Order placed: {new Date(rental.createdAt).toLocaleString()}</div>
                    {rental.updatedAt && rental.updatedAt !== rental.createdAt && (
                      <div>Last updated: {new Date(rental.updatedAt).toLocaleString()}</div>
                    )}
                  </div>
                </div>
              </div>
